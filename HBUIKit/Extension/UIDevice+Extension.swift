...
public extension UIDevice{
    //手机最大设备判断
    class func iphone(_ value: Int) -> Bool{
        let platform = modelPlatform()
        guard let p = platform.components(separatedBy: ",").first, p.contains("iPhone") else{
            return false
        }
        let str = p.replacingOccurrences(of: "iPhone", with: "")
        guard let va = Int(str), va >= value else { return false }
        return true
    }
    
    //手机标识号
    private class func modelPlatform() -> String{
